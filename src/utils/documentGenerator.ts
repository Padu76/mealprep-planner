...
export const generateMealPlanDocument = (
  ...
  console.log('📝 Generating document from:', parsedPlan);
  
  const currentDate = new Date().toLocaleDateString('it-IT');
  const days = parsedPlan.days;
  const firstDay = days[0].meals;
  ...
  days.forEach(dayData => {
    const dayOrderedMeals = getAllMealsInOrder(dayData.meals);
...
export const generateCompleteDocument = generateMealPlanDocument;